...
const newArrayWithDate = newArray.map(item => {
  return [...item, isoDate];
});
...
const createTableQuery = `
    CREATE TABLE IF NOT EXISTS installed_packages_info_updated (
      ...
      installation_info LONGTEXT,
      date VARCHAR(100),
      CONSTRAINT unique_package UNIQUE (package_name, version, architecture,date)
    )`;
...
const insertDataQuery = `
  INSERT INTO installed_packages_info_updated (package_name, distribution, status, version, architecture,installation_info,date)
  VALUES ?
`;



connection.query(insertDataQuery, [newArrayWithDate] ,(err, results) => {
  if (err) {
